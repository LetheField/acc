...
parser.add_argument('--coh_s', type=float, default=5,
                    help='Step for coherence change')
parser.add_argument('--time', type=float, default=1.5,
                    help='Simulation time')
parser.add_argument('--Iconst', type=float, default=0.38, help='Constant current input to ACC')
parser.add_argument('--Iconst_c', action='store_true')
parser.add_argument('--Iconst_s', default=0.005)
parser.add_argument('--pA_ACC', default=1.0)
parser.add_argument('--pA_ACC_c', action='store_true')
parser.add_argument('--pA_ACC_s', default=0.2)
parser.add_argument('--ACC_STN_a', default=0.03)
parser.add_argument('--ACC_STN_a_c', action='store_true')
parser.add_argument('--ACC_STN_a_s', default=0.02)
parser.add_argument('--ACC_STN_n', default=0.03)
parser.add_argument('--ACC_STN_n_c', action='store_true')
parser.add_argument('--ACC_STN_n_s', default=0.02)
parser.add_argument('--Cx_pA', default=0.1)
parser.add_argument('--Cx-pA_c', action='store_true')
parser.add_argument('--Cx_pA_s', default=0.1)
...
modelparams['STN'] = dict(
   ...
   # IFB model parameters
    ...
    gAMPA_E = (opt.ACC_STN_a+(opt.taskID-1)*float(opt.ACC_STN_a_c)*opt.ACC_STN_a_s)*nS, # From ACC
    gNMDA_E = (opt.ACC_STN_n+(opt.taskID-1)*float(opt.ACC_STN_n_c)*opt.ACC_STN_n_s)*nS, # From ACC 
...
modelparams['preA'] = dict(
    ...
    # Scaled recurrent synaptic conductances (onto projection neurons)
    gAMPA_I=(opt.Cx_pA+(opt.taskID-1)*float(opt.Cx_pA_c)*opt.Cx_pA_s)*nS,  # From Cx, this varies from 1.0 to 4.5
    gNMDA_I=0.0*nS,  # From Cx
...
modelparams['ACC'] = dict(
    ...
    # Scaled recurrent synaptic conductances (onto projection neurons)
    gGABA_E=(opt.pA_ACC+(opt.taskID-1)*float(opt.pA_ACC_c)*opt.pA_ACC_s)*nS,       # From preA
...
class Model(object):
    def __init__(self, modelparams, stimulus, T):
        ...
        for x in ['E', 'I']:
            net['pg'+x] = PoissonGroup(params['Cx']['N_'+x], params['Cx']['nu_ext'])
            net['ic'+x] = Synapses(net['pg'+x], net['Cx'+x], on_pre='sAMPA_ext += 1', delay=delay)
            net['ic'+x].connect(condition='i == j')

        for x in ['Str', 'SNr', 'STN', 'SCE', 'SCI', 'preA', 'ACC']:
            net['pg'+x] = PoissonGroup(params[x]['N_PJ'], params[x]['nu_ext'])
            net['ic'+x] = Synapses(net['pg'+x], net[x], on_pre='sAMPA_ext += 1', delay=delay)
            net['ic'+x].connect(condition='i == j')

        net['pg'+'GPe_AMPA'] = PoissonGroup(params['GPe']['N_PJ'], params['GPe']['nu_ext_AMPA'])
        net['ic'+'GPe_AMPA'] = Synapses(net['pg'+'GPe_AMPA'], net['GPe'], on_pre='sAMPA_ext += 1', delay=delay)
        net['ic'+'GPe_AMPA'].connect(condition='i == j')

        net['pg'+'GPe_GABA'] = PoissonGroup(params['GPe']['N_PJ'], params['GPe']['nu_ext_GABA'])
        net['ic'+'GPe_GABA'] = Synapses(net['pg'+'GPe_GABA'], net['GPe'], on_pre='sGABA_ext += 1', delay=delay)
        ...
        for x in ['CxI', 'SNr', 'GPe', 'SCI', 'Str', 'preA']:
            net['icGABA_'+x] = Synapses(net[x], net[x], on_pre='sGABA += 1', delay=delay)
        ...
        def recurrent_input():
            ...
            for x in ['Cx', 'STN']:
                for i in ['1', '2']:
                    SAMPA[i][x] = sum(self.netPJsub[x+i].sAMPA) # Sum for all neurons in the group
            ...
            for i in range(3):
                self.netPJsub['Cx'+str(i)].S_NMDA = S[i] + self.wNMDA_SCE_CxE*(SNMDA['1']['SCE']+SNMDA['2']['SCE'])
            ...
            for i in ['1','2']:
                ...
                # Str
                self.netPJsub['Str'+i].S_AMPA = dot(self.netPJsub['Cx'+i].D, self.netPJsub['Cx'+i].sAMPA)
                ...
                # GPe
                self.netPJsub['GPe'+i].S_AMPA = self.sconn_STN_GPe.dot(array(self.netPJsub['STN'+i].sAMPA))
                self.netPJsub['GPe'+i].S_NMDA = self.sconn_STN_GPe.dot(array(self.netPJsub['STN'+i].sNMDA))
                self.netPJsub['GPe'+i].S_GABA = self.sconn_GPe_GPe.dot(array(self.netPJsub['GPe'+i].sGABA))*self.wGABA_GPe_GPe + SGABA[i]['Str']
                # STN:
                self.netPJsub['STN'+i].S_GABA = self.sconn_GPe_STN.dot(array(self.netPJsub['GPe'+i].sGABA))
        ...
        for ind, sname in zip([1, 2], ['s1', 's2']):
            net['pg'+str(ind)] = PoissonGroup(params['Cx']['N'+str(ind)], '%s(t)' % sname)
            net['ic'+str(ind)] = Synapses(net['pg'+str(ind)], netPJsub['Cx'+str(ind)], on_pre='sAMPA_ext += 1', delay=delay)
        ...
        rates = OrderedDict()
        for x in netPJsub:
            rates[x] = PopulationRateMonitor(netPJsub[x]) # bin have to be changed
...
class Simulation(object):
    ...
    def saverates(self, filename):
        ...
        for name in ['Cx', 'Str', 'SNr', 'GPe', 'STN', 'SCE']:
            rates[name+'1'] = self.model.rates[name+'1'].smooth_rate(width=5*ms)/Hz
            rates[name+'2'] = self.model.rates[name+'2'].smooth_rate(width=5*ms)/Hz
...
stimparams = dict(
    ...
    muA = 60*Hz,
    muB = 20*Hz,
    coh=opt.coh+(float(opt.coh_c)*(opt.taskID-1)*opt.coh_s)         # Percent coherence
)
sim_dt = 0.05*ms
T = opt.time*second
...
if __name__ == '__main__':
    ...
    plots = {
        ...
        'preA': fig.add_axes([x2, y2, w, h]),
        'ACC': fig.add_axes([x3, y2, w, h])
        }
    ...
    for name, plot in plots.items():
        ...
        plot.set_xlim(-100, 900)
        plot.set_xticks([0, 300, 600, 900])
